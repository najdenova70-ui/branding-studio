...
The web category contains two local Figma renders: home `4206:201963` (1312×760) and course detail `8444:23884` (1335×812, программа обучения «Креативное мышление»). On home, only the source image of banner instance `6759:41703` is replaceable; its text, controls, placement, and geometry stay fixed. Clicking the fixed course card on home opens the course detail view. The course screen has no editable elements.

Web now starts with the code-rendered authorization screen from Figma `8470:8952` (1280×720). Its project-scoped assets are independent from mobile authorization: `web.authorization.background` maps to `Background Authorization / Web` (`8467:2264`, 1024×1540), and `web.authorization.logo` maps to `Logo authorization WEB` (`8467:2291`, 800×600). The web screen registry lives in `src/registry/WebScreenRegistry.ts`; the current-screen inspector exposes only the asset paths declared by the selected web screen. Future web assets are confirmed but intentionally not connected yet: `Background / Navigation Web` (`8467:2265`), `Logo / Navbar rus` (`8467:2282`), and favicon nodes `8467:2266–2272`.
